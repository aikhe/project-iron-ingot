...
function BlogCardWithCursor({ row, cardBaseClass, borderClass, setHoveredCard, handleMouseMove, isCursorHidden }) {
...
function ArticleCardWithCursor({ row, cardBaseClass, borderClass, setHoveredCard, handleMouseMove, isCursorHidden }) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className={`flex-1 relative overflow-hidden border-t border-[#2A2A2A] ${row.articleBg || 'bg-[#242424]'}`}>
          {row.articleImage && (
            <Image
          ...
          <div className="absolute bottom-6 left-6 z-10 flex flex-wrap gap-2">
            {row.articleTags && row.articleTags.map((tag) => (
              <span
                key={tag}
...
export default function LatestOnIngoCards({ blog, thesis, bulletin }) {
  ...
  const rows = [
    ...
    },
    {
      ...
      articleTags: thesis?.tags?.slice(0, 3) || ["WEBSITE", "E-LEARNING", "EDUCATION"],
      articleBg: "bg-[#181818]",
  ...
  return (
    <div className="w-full font-sans bg-[#181818] relative overflow-hidden">
      ...
      {rows.map((row, idx) => {
        ...
        const BlogCard = (
          <Link key={`blog-link-${row.id}`} href={row.href} className="contents">
            <BlogCardWithCursor
              row={row}
        ...
        const ArticleCard = (
          <Link key={`article-link-${row.id}`} href={row.href} className="contents">
        ...
        return (
          <div
            key={row.id}
            className={`w-full border-b border-dashed border-[#2A2A2A] transition-colors ${idx === 0 ? "border-t" : "-mt-px"
              }`}
          >
