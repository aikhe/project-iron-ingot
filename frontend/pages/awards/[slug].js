...
const validateSanityData = (data, slug) => {
  ...
  if (data._id && !data._id.startsWith("drafts.") && !data.title) {
    warnings.push(
      "Document exists but title is missing - possible incomplete save",
    );
  }
  ...
  if (data.recipients && Array.isArray(data.recipients)) {
    data.recipients.forEach((recipient, index) => {
      if (
        ...
        !recipient.recipientPhoto.includes("cdn.sanity.io")
      ) {
        warnings.push(
          `Recipient ${index + 1} photo may not be properly uploaded to Sanity CDN`,
        );
      }
    });
  }
...
const generateSampleAwardData = (slug) => {
  const sampleData = {
    "excellence-award-2024": {
      ...
      headerImage:
        "https://cdn.sanity.io/images/sample/800x600/award-ceremony.jpg",
      ...
      recipients: [
        {
          ...
          batchYear: "2024",
          recipientPhoto:
            "https://cdn.sanity.io/images/sample/200x200/juan-dela-cruz.jpg",
        },
        {
          ...
          batchYear: "2024",
          recipientPhoto:
            "https://cdn.sanity.io/images/sample/200x200/maria-santos.jpg",
        },
      ],
      content: [
        {
    ...
    "invalid-data-example": {
      ...
      category:
        "This category name is way too long and exceeds the maximum allowed length limit",
...
const AwardPage = ({
  ...
  useEffect(() => {
    if (awardPost) {
      ...
      const clientValidation = validateSanityData(
        awardPost,
        debugInfo?.slug || "unknown",
      );
              ...
              {isDemoMode && (
                <div className="bg-blue-600/20 border border-blue-500/40 rounded-lg p-4 mb-6">
                  ...
                  <p className="text-blue-200 text-sm mt-2">
                    ...
                    because the Sanity CMS is not available.
                  </p>
                </div>
              )}
          ...
          <div className="w-full bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-b border-blue-500/30 py-3">
            <div className="container mx-auto px-5 text-center">
              <p className="text-blue-300 text-sm">
                <span className="font-semibold">Demo Mode:</span> This page is
                displaying sample validation data for demonstration purposes.
              </p>
            </div>
          </div>
        )}
            ...
            <div className="flex flex-col gap-2">
              {post?.category && (
                <p className="text-white/80">
                  ...
                  {renderSafeText(post.category, 30)}
                </p>
              )}
              {post?.description && (
                        ...
                        >
                          {recipient?.recipientPhoto && (
                            <div className="relative w-12 h-12 rounded-full overflow-hidden bg-gray-600">
                              <Image
                                ...
                                onError={(e) => {
                                  console.warn(
                                    "Recipient photo failed to load:",
                                    recipient.recipientPhoto,
                                  );
                                  e.target.style.display = "none";
                                }}
                          ...
                          <div>
                            ...
                            <p className="text-sm text-white/60">
                              {recipient.yearLevel &&
                                renderSafeText(recipient.yearLevel, 15)}
                              {recipient.yearLevel &&
                                recipient.batchYear &&
                                " - "}
                              {recipient.batchYear &&
                                `Batch ${recipient.batchYear}`}
                            </p>
                          </div>
                        </div>
                      );
